...
import {
  ...
  getMovimientosInventarioById,
  updateMovimientosInventarioById,
} from '../services/movimientoInventarioServices';
import {
  Almacen,
  ProductoResponse,
...
export const MovimientoInventarioEdit = () => {
  ...
  const handleSendForm = async () => {
    ...
    const formattedDate = dayjs(formData.fecha).format(
      'YYYY-MM-DDTHH:mm:ss.SSSSSS-06:00'
    );
    ...
    if (params.id) {
      try {
        ...
        const response = await updateMovimientosInventarioById(
          params.id,
          updatedFormData
        );
        console.log(response);
