...
export const getAllMovimientosInventario = async (
  ...
  try {
    ...
    const movimientosConNombreProducto = await Promise.all(movimientos.map(async (movimiento: any) => {
...
export const getMovimientosInventarioById = async (id: string | number) => {
  try {
    const response = await apiInventory.get<resultsMovimiento>(`/movimientos-inventario/${id}/`,);
