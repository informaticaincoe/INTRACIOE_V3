import { useEffect, useState } from 'react'
import { Title } from '../../../../shared/text/title'
import { WhiteSectionsPage } from '../../../../shared/containers/whiteSectionsPage'
import { getAllDevolucionesVentas } from '../services/devolucionVentaServices'
import { TablaDevolucionesVenta } from '../componentes/tablaDevolucionesVenta'
import { useSearchParams } from 'react-router'
import { Pagination } from '../../../../shared/interfaces/interfacesPagination'
import { DevolucionVentaInterface } from '../interface/devolucionVentaInterface'

export const DevoluacionesVentaPage = () => {
    const [searchParams, setSearchParams] = useSearchParams();
    const [loading, setLoading] = useState(false)
    const [devolucionesVenta, setDevolucionesVenta] = useState<DevolucionVentaInterface>({
        count: 1,
        current_page: 1,
        page_size: 10,
        has_next: true,
        has_previous: false,
        results: []
    })
    const [pagination, setPagination] = useState<Pagination>({
        count: 1,
        current_page: 1,
        page_size: 10,
        has_next: true,
        has_previous: false,
    });

    useEffect(() => {
        fetchVentas()
    }, [])

    useEffect(() => {
        // Reinicia a la página 1 cada vez que los filtros cambian
        setPagination((prev: any) => ({ ...prev, current_page: 1 }));
        // Se utiliza el page_size actual para la consulta
        fetchVentas(1, pagination.page_size); //enviar el numero de pagina actual 1 por defecto, enviar la cantidad de elementos en pagina
    }, []);

    const updateDevolucionVentas = () => {
        fetchVentas(pagination.current_page);
    };

    const onPageChange = (event: any) => {
        const page = event.page + 1;
        const limit = event.rows;
        fetchVentas(page, limit);
    };

    const fetchVentas = async (page = 1, limit = 10) => {
        try {
            setLoading(true)
            const response = await getAllDevolucionesVentas({ page, limit })
            if (response) {
                setDevolucionesVenta(response)
                setPagination({
                    count: response.count || 0,
                    current_page: response.current_page || 1,
                    page_size: response.page_size || limit,
                    has_next: response.has_next,
                    has_previous: response.has_previous
                });

                const params: Record<string, string> = {
                    page: String(response.current_page),
                    page_size: String(response.page_size),
                    // date_from: initialDateFrom,        // <-- futuro: filtro fecha
                    // date_to:   initialDateTo,
                };
                setSearchParams(params, { replace: true });
            } else {
                setPagination({
                    count: 1,
                    current_page: 1,
                    page_size: 10,
                    has_next: true,
                    has_previous: false,
                });
            }
        } catch (error) {
            console.log(error)
        }
    }


    return (
        <>
            <Title text={'Devoluciones de ventas'} />

            <WhiteSectionsPage>
                <>
                    {/* <TablaComprasHeader codigo={codigoFiltro} onSearch={handleSearch} />
                    <Divider /> */}
                    <TablaDevolucionesVenta
                        updateList={updateDevolucionVentas}
                        devolucionesList={devolucionesVenta.results}
                        pagination={pagination}
                        onPageChange={onPageChange}
                    />
                </>
            </WhiteSectionsPage>
        </>
    )
}
