...
export const FilterContainer: React.FC<FilterContainerProps> = ({ filters, setFilters }) => {
    // Inicialmente, el array contiene la opción "Todos"
    const [tipoDocumento, setTipoDocumento] = useState<any[]>([{ name: "Todos", code: null }]);

    useEffect(() => {
        const fetchTipoDte = async () => {
            try {
                const response = await getAllTipoDte();
                const opciones = response.map((documento: { descripcion: any; id: any }) => ({
    ...
    return (
        <div className='flex gap-10 pb-10 pt-5'>
            <span>
