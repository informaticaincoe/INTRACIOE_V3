...
import { getAllFacturasSujetoExcluido } from '../services/listadoFacturasServices';
...
export const ListadoFacturasSujetoExcluido = () => {
  ...
  const fetchFacturas = async (page = 1, limit = 20) => {
    try {
      const response = await getAllFacturasSujetoExcluido({ page, limit, filters });
      if (response) {
