...
import { generarFacturaService, generarFacturaSujetoExcluidoService } from '../../services/facturavisualizacionServices';
...
import { InformacionsujetoExcluido } from '../../components/SujetoExcluido/informacionSujetoExcluido';
import { TablaProductosSujetoExcluido } from '../../components/SujetoExcluido/tablaProductosSujetoExcluido';
import { ResumenSujetoExcluido, ResumenSujetoExcluidoDefault, SujetoExcluido, SujetoExcluidoDefault } from '../../interfaces/sujetoExcluidoInterfaces';
import { CuerpoDocumentoSujetoExcluido, CuerpoDocumentoSujetoExcluidoDefault, DatosFactura, DatosFacturaDefault, Emisor, EmisorDefault, ResumenDefalt } from '../../interfaces/facturaPdfInterfaces';
import { TablaResumenSujetoExcluido } from '../../components/SujetoExcluido/tablaResumenSujetoExcluido';

interface SujetoExcluidoPageProps {
  id: string
}

export const SujetoExcluidoPage: React.FC<SujetoExcluidoPageProps> = ({ id }) => {
  const [emisor, setEmisor] = useState<Emisor>(EmisorDefault);
  const [sujetoExcluido, setSujetoExcluido] = useState<SujetoExcluido>(SujetoExcluidoDefault);
  const [datosFactura, setDatosFactura] = useState<DatosFactura>(DatosFacturaDefault);
  const [productos, setProductos] = useState<CuerpoDocumentoSujetoExcluido[]>([CuerpoDocumentoSujetoExcluidoDefault]);
  const [resumen, setResumen] = useState<ResumenSujetoExcluido>(ResumenSujetoExcluidoDefault);
  ...
  const fetchDatosFactura = async () => {
    try {
      if (id) {
        const response = await generarFacturaSujetoExcluidoService(id);

        setEmisor(response.emisor);
        setDatosFactura(response.datosFactura);
        if (response.sujetoExcluido) {
          setSujetoExcluido(response.sujetoExcluido);
        }
    ...
    <>
      ...
      >
        ...
        >
          ...
          />
          <InformacionsujetoExcluido sujetoExcluido={sujetoExcluido} />

          <div className="flex flex-col">
            <TablaProductosSujetoExcluido
              productos={productos}
            ...
            <div id="footer">
              <div className="grid grid-cols-2 gap-x-5 pt-5">
                ...
                <span>
                  <TablaResumenSujetoExcluido resumen={resumen} />
                </span>
              </div>
              <span className='flex justify-between pt-3'>
                <p className='text-nowrap'>version: {datosFactura.version}</p>
